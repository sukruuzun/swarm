#!/usr/bin/env python3
"""
Birleşik Parisi-Nash v4 Deneyi: Differentiable Routing
=========================================================
v3'teki gradyan körlüğünü çözen kritik düzeltme.

v3 Sorunu: top_k=1 ile router olasılığı kullanılmıyordu
  → output = expert_out (p_k yok → router'a SIFIR gradyan)
  → Expert kullanımı %25/%25/%25/%25'te sabit kaldı

v4 Çözümü: output = p_k × expert_out (differentiable)
  → CE loss → ∂L/∂p_k → router ÖĞRENİR
  → Expert'ler içeriğe göre UZMANLAŞIR

Kullanım:
    PYTHONUNBUFFERED=1 python3 -u experiment_unified_v4.py
"""
...
def main():
    ...
    print()
    print("╔" + "═" * 68 + "╗")
    print("║  BİRLEŞİK PARİSİ-NASH v4: DIFFERENTIABLE ROUTING                  ║")
    print("║  v_i = p_k · φ_k(Σ_j∈N(i) α_ij^k · V_k_j) + η·ε                  ║")
    print("╚" + "═" * 68 + "╝")
    print()
    print("  v4 Kritik Düzeltme:")
    print("    ✓ Differentiable routing: output = p_k × expert_out")
    print("    ✓ CE loss → router'a gradyan AKAR → uzmanlaşma")
    print("    ✓ Temperature Annealing 2.0 → 0.3")
    print("    ✓ LB katsayısı 0.001")
    print()
    ...
    print(f"  ⑥ BİRLEŞİK v4 Parametreleri: {total_p:,} ({total_p/1e6:.1f}M)")
    ...
    print()
    print("╔" + "═" * 68 + "╗")
    print("║  SONUÇ: BİRLEŞİK v4 vs TÜM MODELLER                              ║")
    print("╚" + "═" * 68 + "╝")
    print()
    ...
    print(f"  {'⑥ BİRLEŞİK v4 (diff. routing)':<45s} "
    ...
    if best_ppl < 302.6:
        ...
        print(f"  ★ v3'ü GEÇTİ! PPL {best_ppl:.1f} < 443.1")
    else:
        print(f"  v3 seviyesinde: PPL {best_ppl:.1f}")

    # Kaydet
    result = {
        'name': 'BİRLEŞİK v4 (differentiable routing)',
        'params': total_p,
        'final_ppl': final_ppl,
        'best_ppl': best_ppl,
        'time': total_time,
    ...
    with open("results_unified_v4.json", 'w') as f:
        json.dump(result, f, indent=2, default=str)
    print(f"\n  Sonuçlar: results_unified_v4.json")
    print()
